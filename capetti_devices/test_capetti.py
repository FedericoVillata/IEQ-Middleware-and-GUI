...
class CapettiAPI:
    def __init__(self, username, rest_license,capetti_url):
        self.username = username
        self.rest_license = rest_license
        self.base_url = capetti_url
        self.publisher = MyPublisher("Capetti")
        self.token = None
        self.first_request = True
        self.channel_type_mapping = {
    ...
    def get_sensor_list(self, mac, apartment_id, sensor_room_mapping):
        if not self.token:
        ...
        if response.status_code == 401:  # Token expired
            ...
            self.get_user_token()
            return self.get_sensor_list(mac, apartment_id, sensor_room_mapping)  # Retry with new token
        ...
        if self.first_request:
            ...
            date_from = date_to - int(timedelta(days=2).total_seconds())  
            ...
            while current_start <= date_to:
                ...
                self.get_history_values(date_from=current_start, date_to=current_end, mac=mac, apartmentId=apartment_id, sensors_room_mapping=sensor_room_mapping)
                current_start = current_end
                time.sleep(3)
            print("End of history data fetching")
        else:
            self.get_current_values(mac, apartment_id, sensor_room_mapping)

    def get_history_values(self, date_from, date_to, mac, apartmentId, sensors_room_mapping, start_index=0):
        if not self.token:
            print("Error: User token not available.")
            return
        ...
        pubTopic = f"IEQmidAndGUI/{apartmentId}/sensorData"
        ...
        for index, item in enumerate(data[start_index:], start=start_index):
            ...
            if invalid == 1 or alarm == 1 or value == '':
                continue
            if channel_type not in self.channel_type_mapping:
                continue

            measure_type, unit = self.channel_type_mapping[channel_type]

            if sensor_mac in sensors_room_mapping:
                room_id = sensors_room_mapping[sensor_mac]
        ...
        print(f"Publishing message: with {len(Event)} events")
        capetti.publisher.myPublish(json.dumps(out), pubTopic)
            # Reset the Event list after publishing

        # Salva l'indice corrente in caso di errore
        start_index = index
            
    def get_current_values(self, mac, apartmentId, sensors_room_mapping):
        pubTopic = f"IEQmidAndGUI/{apartmentId}/sensorData"
        if not self.token:
            print("Error: User token not available.")
            return
...
if __name__ == '__main__':
    ...
    while True:  
        capetti.publisher.start()
        apartments = get_apartments(registry_url=config["registry_url"])
        print("Apartments fetched from registry:")
        ...
        for apartment in apartments:
            ...
            capetti.get_sensor_list(mac_address, apartment_id, sensor_room_mapping)
        
        capetti.first_request = False
        capetti.publisher.stop()
